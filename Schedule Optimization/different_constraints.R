...
# Penalty 7: Minimize the maximum number of night shifts each person has
...
get_energy_sub <- function(subsched, penalties) {
  ...
  # Array to keep track of night shifts for each worker
  ...
  for (i in 1:dim(subsched)[1]) {
    ...
    # Track night shifts for this worker (assuming night shift is the last shift)
    night_shifts[i] <- sum(subsched[i,,nshifts] == 1)
    ...
    for (j in 1:ndays) {
      ...
      if (all(subsched[i, j,] == FALSE)) {
        ...
        if (!prevoff && j == 2) {  # that's a penalty (treating first two days as the weekend)
  ...
  # Penalize the schedule based on the maximum number of night shifts any worker has
  max_night_shifts <- max(night_shifts)
  energy <- energy + penalties[7] * max_night_shifts  # Add penalty for maximum night shifts
