...
simulate_multiple_brownian_motions <- function(T, n, num_paths) {
  ...
  for (i in 1:num_paths) {
    increments <- rnorm(n, mean = 0, sd = sqrt(T / n)) 
...
colors <- sapply(1:num_paths, function(i) {
  ...
  # Choose color based on whether the final value is positive (green)
  # or negative (red)
  if (final_value > 0) {
    return(rgb(0.6, 1, 0.6, alpha = 0.9))
  } else {
    return(rgb(1, 0.6, 0.6, alpha = 0.9))
  }
})
